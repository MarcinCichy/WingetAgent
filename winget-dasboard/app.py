...
@app.route('/')
def index():
    computers = get_db().execute("SELECT id, hostname, ip_address, last_report, reboot_required FROM computers ORDER BY hostname COLLATE NOCASE").fetchall()
...
def view_report(report_id):
    db = get_db()
    report = db.execute("SELECT r.id, r.report_timestamp, c.hostname, c.ip_address FROM reports r JOIN computers c ON r.computer_id = c.id WHERE r.id = ?", (report_id,)).fetchone()
...
def receive_report():
    ...
    try:
        ...
        if computer:
            ...
            cur.execute("UPDATE computers SET ip_address = ?, reboot_required = ?, last_report = CURRENT_TIMESTAMP WHERE id = ?", (data.get('ip_address'), data.get('reboot_required', False), computer_id))
        else:
            cur.execute("INSERT INTO computers (hostname, ip_address, reboot_required) VALUES (?, ?, ?)", (hostname, data.get('ip_address'), data.get('reboot_required', False)))
            computer_id = cur.lastrowid
        previous_report = db.execute("SELECT id FROM reports WHERE computer_id = ? ORDER BY report_timestamp DESC LIMIT 1", (computer_id,)).fetchone()
        ...
        if apps_to_insert: cur.executemany("INSERT INTO applications (report_id, name, app_id, version) VALUES (?, ?, ?, ?)", apps_to_insert)
        ...
        if app_updates_to_insert: cur.executemany("INSERT INTO updates (report_id, name, app_id, current_version, available_version, update_type) VALUES (?, ?, ?, ?, ?, 'APP')", app_updates_to_insert)
        ...
        if os_updates_to_insert: cur.executemany("INSERT INTO updates (report_id, name, available_version, update_type) VALUES (?, ?, ?, 'OS')", os_updates_to_insert)
        if previous_report:
            old_updates_q = db.execute("SELECT name FROM updates WHERE report_id = ? AND update_type = 'OS'", (previous_report['id'],)).fetchall()
            ...
            for update_name in installed_updates:
                cur.execute("INSERT INTO action_history (computer_id, action_type, details) VALUES (?, ?, ?)", (computer_id, 'OS_UPDATE_SUCCESS', json.dumps({"name": update_name})))
        db.commit()
    except Exception as e:
        db.rollback(); logging.error(f"Krytyczny błąd podczas przetwarzania raportu od {hostname}: {e}", exc_info=True)
...
def request_uninstall(computer_id):
    ...
    db.execute("INSERT INTO tasks (computer_id, command, payload) VALUES (?, ?, ?)",(computer_id, 'uninstall_package', data.get('package_id')))
    db.commit()
...
def task_result():
    ...
    latest_report_q = db.execute("SELECT id FROM reports WHERE computer_id = ? ORDER BY report_timestamp DESC LIMIT 1", (computer_id,)).fetchone()
    ...
    if command == 'update_package':
        app_details = db.execute("SELECT name, current_version, available_version FROM updates WHERE report_id = ? AND app_id = ?", (latest_report_id, package_id)).fetchone()
        ...
        if status == 'zakończone' and app_details:
            ...
            details_dict = {"name": app_name, "from": app_details['current_version'], "to": app_details['available_version']}
        else:
            action_type = 'APP_UPDATE_FAILURE'
            details_dict = {"name": app_name}
            db.execute("UPDATE updates SET status = 'Niepowodzenie' WHERE report_id = ? AND app_id = ?", (latest_report_id, package_id))
    elif command == 'uninstall_package':
        app_info = db.execute("SELECT name FROM applications WHERE report_id = ? AND app_id = ?", (latest_report_id, package_id)).fetchone()
    ...
    if action_type:
        db.execute("INSERT INTO action_history (computer_id, action_type, details) VALUES (?, ?, ?)", (computer_id, action_type, json.dumps(details_dict)))
    db.commit()
...
def report_snapshot(report_id):
    report = get_db().execute("SELECT c.hostname FROM reports r JOIN computers c ON r.computer_id = c.id WHERE r.id = ?", (report_id,)).fetchone()
    if not report: abort(404)
...
def generate_report_content(computer_ids):
    ...
    for cid in computer_ids:
        ...
        history = db.execute("SELECT timestamp, action_type, details FROM action_history WHERE computer_id = ? ORDER BY timestamp DESC LIMIT 20", (cid,)).fetchall()
        if history:
            for item in history:
                details = json.loads(item['details'])
                log_entry = f"* [{to_local_time_filter(item['timestamp'])}] "
                if item['action_type'] == 'APP_UPDATE_SUCCESS': log_entry += f"Sukces aktualizacji: {details.get('name', '')} (z {details.get('from', '?')} do {details.get('to', '?')})"
                elif item['action_type'] == 'APP_UPDATE_FAILURE': log_entry += f"Błąd aktualizacji: {details.get('name', '')}"
                elif item['action_type'] == 'APP_UNINSTALL_SUCCESS': log_entry += f"Sukces deinstalacji: {details.get('name', '')}"
                elif item['action_type'] == 'APP_UNINSTALL_FAILURE': log_entry += f"Błąd deinstalacji: {details.get('name', '')}"
                elif item['action_type'] == 'OS_UPDATE_SUCCESS': log_entry += f"Sukces aktualizacji systemu: {details.get('name', '')}"
                content.append(log_entry)
        else: content.append("* Brak zarejestrowanych zdarzeń w historii.")
...
def generate_snapshot_report_content(report_id):
    ...
    updates = db.execute("SELECT name, current_version, available_version, update_type FROM updates WHERE report_id = ?", (report_id,)).fetchall()
    if updates:
        for item in updates:
            if item['update_type'] == 'OS': content.append(f"* [System] {item['name']} (KB: {item['available_version']})")
            else: content.append(f"* [Aplikacja] {item['name']}: {item['current_version']} -> {item['available_version']}")
    else: content.append("* Brak.")
